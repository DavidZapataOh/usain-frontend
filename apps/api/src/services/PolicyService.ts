...
export class PolicyService {
  ...
  async getPolicies(params?: PaginationParams): Promise<PaginatedResponse<Policy>> {
    try {
      ...
      throw new SDKError(
        `Failed to get policies: ${(error as Error).message}`,
        'POLICIES_LIST_ERROR',
        500,
  ...
  async getPolicy(policyId: string): Promise<Policy> {
    try {
      ...
      throw new SDKError(
        `Failed to get policy: ${(error as Error).message}`,
        'POLICY_GET_ERROR',
        500,
        { policyId, error: (error as Error).message }
      );
    }
  }
  ...
  async pausePolicy(policyId: string): Promise<Policy> {
    try {
      ...
      throw new SDKError(
        `Failed to pause policy: ${(error as Error).message}`,
        'POLICY_PAUSE_ERROR',
        500,
        { policyId, error: (error as Error).message }
      );
  ...
  async resumePolicy(policyId: string): Promise<Policy> {
    try {
      ...
      throw new SDKError(
        `Failed to resume policy: ${(error as Error).message}`,
        'POLICY_RESUME_ERROR',
        500,
        { policyId, error: (error as Error).message }
      );
  ...
  async revokePolicy(policyId: string): Promise<void> {
    try {
      ...
      throw new SDKError(
        `Failed to revoke policy: ${(error as Error).message}`,
        'POLICY_REVOKE_ERROR',
        500,
        { policyId, error: (error as Error).message }
      );
  ...
  async validatePolicy(
    ...
    try {
      ...
      throw new SDKError(
        `Failed to validate policy: ${(error as Error).message}`,
        'POLICY_VALIDATION_ERROR',
        500,
        { policyId, intent, error: (error as Error).message }
      );
    }
  }
  ...
  async getActivePolicy(): Promise<Policy | null> {
    try {
      ...
      throw new SDKError(
        `Failed to get active policy: ${(error as Error).message}`,
        'ACTIVE_POLICY_ERROR',
        500,
  ...
  async getPolicyStats(policyId: string): Promise<{
    ...
    try {
      ...
      throw new SDKError(
        `Failed to get policy stats: ${(error as Error).message}`,
        'POLICY_STATS_ERROR',
        500,
        { policyId, error: (error as Error).message }
      );

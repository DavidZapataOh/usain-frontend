...
export class IntentService {
  private vincentClient: VincentClient;
  ...
  async submitIntent(request: SubmitIntentRequest): Promise<SubmitIntentResponse> {
    ...
    try {
      ...
      // 2. Validate policy
      const policyValidation = await this.vincentClient.validatePolicy(
        request.policyId,
        {
      ...
      if (!policyValidation.isValid) {
        throw new ValidationError(
          `Policy validation failed: ${policyValidation.reason}`,
          { policyId: request.policyId, reason: policyValidation.reason }
        );
      }
      ...
      throw new SDKError(
        `Failed to submit intent: ${(error as Error).message}`,
        'INTENT_SUBMIT_ERROR',
        500,
  ...
  private async verifySignature(intent: Intent, signature: string): Promise<boolean> {
    try {
  ...
  private async updateChannelState(intent: Intent): Promise<{
    channelId: string;
    txHash?: string;
  }> {
    try {
      ...
      throw new SDKError(
        `Failed to update channel state: ${(error as Error).message}`,
  ...
  private async storeFillMetrics(
  ...
  async getIntentStatus(fillId: string): Promise<{
    fillId: string;
    status: string;
    latencyMs: number;
    timestamp: number;
    ...
    try {
      ...
      throw new SDKError(
        `Failed to get intent status: ${(error as Error).message}`,
        'INTENT_STATUS_ERROR',
        500,
        { fillId, error: (error as Error).message }
      );
    }
  }
}

...
export default function ChannelPage() {
  ...
  const handleRequestSettlement = async () => {
    try {
      ...
      showSuccess('Settlement requested successfully!')
      ...
      console.debug('[Channel] Settlement request failed:', error)
      showError('Failed to request settlement. Please try again.', error)
    }
  }
          ...
          {channel?.status === "OPEN" ? (
            <>
              <button
                ...
                className="bg-primary text-primary-foreground rounded-xl font-semibold hover:brightness-95 active:scale-[0.98] transition-all px-6 py-3 flex-1 focus:ring-2 focus:ring-primary/50"
                aria-label="Request settlement for current channel state"
              >
                Request Settlement
              </button>
              <button
